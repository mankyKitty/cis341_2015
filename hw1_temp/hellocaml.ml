...
let compose_pair (p:(('b -> 'c) * ('a -> 'b))) : 'a -> 'c =
  match p with | (f,g) -> fun (a:'a) -> f (g a)
...
let rev_t (l: 'a list) : 'a list =
  let rec rev_t' (xs: 'a list) (acc: 'a list) : 'a list =
    match xs with
    | [] -> acc
    | (h :: t) -> rev_t' t (h :: acc)
  in
...
let rec vars_of (e:exp) : string list =
  let rec vars' (e:exp) (l:string list): string list =
    match e with
    | Var v -> v :: l
    | Const _ -> l
    | Add (a,b) -> union (vars' a l) (vars' b l)
    | Mult (a,b) -> union (vars' a l) (vars' b l)
    | Neg e -> vars' e l
  in
  vars' e []
...
let rec lookup (x:string) (c:ctxt) : int32 =
  match c with
  | [] -> raise Not_found
  | ((n,v)::t) -> if n = x then v else lookup x t
...
let rec interpret (c:ctxt) (e:exp) : int32 =
  ...
  | Var v -> lookup v c
  | Const n -> n
  | Add (a,b) -> Int32.add (interpret c a) (interpret c b)
  | Mult (a,b) -> Int32.mul (interpret c a) (interpret c b)
...
let rec optimize (e:exp) : exp =
  ...
  | Const _ -> e
    (* Add Optimisations *)
  | Add (Const 0l, b) -> optimize b
  | Add (a, Const 0l) -> optimize a
  | Add (Const a, Const b) -> Const (Int32.add a b)
  | Add (Const a, b) -> begin match optimize b with
    | Const b' -> Const (Int32.add a b')
    | b' -> Add (Const a, b')
    end
  | Add (a, Const b) -> begin match optimize a with
    | Const a' -> Const (Int32.add a' b)
    | a' -> Add (a', Const b)
    end
  | Add (_,_) -> e
  (* Mult optimisations *)
  | Mult (Const 0l, b) -> Const 0l
  | Mult (a, Const 0l) -> Const 0l
  | Mult (Const a, Const b) -> Const (Int32.add a b)
  | Mult (_,_) -> e
...
let rec compile (e:exp) : program =
  let rec compile' (e:exp) (p:program) : program =
    begin match e with
      | Var x -> IPushV x :: p
      | Const c -> IPushC c :: p
      | Add (a,b) -> compile b @ compile a @ (IAdd :: p)
      | Mult (a,b) -> compile b @ compile a @ (IMul :: p)
      | Neg e -> compile e @ (INeg :: p)
    end
  in
  compile' e []
